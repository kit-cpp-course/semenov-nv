...
void AES::mix_columns(unsigned char *buffer) {
	...
	for (i = 0; i < 16; i += 4) {
		e = buffer[i] ^ buffer[i + 1] ^ buffer[i + 2] ^ buffer[i + 3];
		mix(buffer, e, e, i);
	}
}

void AES::mix_columns_inv(unsigned char *buffer) {
	unsigned char i, e, x, y, z;

	for (i = 0; i < 16; i += 4) {
		e = buffer[i] ^ buffer[i + 1] ^ buffer[i + 2] ^ buffer[i + 3];
		z = rj_xtime(e);
		x = e ^ rj_xtime(rj_xtime(z ^ buffer[i] ^ buffer[i + 2]));
		y = e ^ rj_xtime(rj_xtime(z ^ buffer[i + 1] ^ buffer[i + 3]));

		mix(buffer, x, y, i);
	}
}

void AES::mix(unsigned char *buffer, unsigned char & x, unsigned char & y, int i)
{
	buffer[i] ^= x ^ rj_xtime(buffer[i] ^ buffer[i + 1]);
	buffer[i + 1] ^= y ^ rj_xtime(buffer[i + 1] ^ buffer[i + 2]);
	buffer[i + 2] ^= x ^ rj_xtime(buffer[i + 2] ^ buffer[i + 3]);
	buffer[i + 3] ^= y ^ rj_xtime(buffer[i + 3] ^ buffer[i]);
